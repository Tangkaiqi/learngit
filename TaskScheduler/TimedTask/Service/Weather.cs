...
namespace TimedTask.Service
{
    ...
    {
        private readonly string _filePath = Model.PM.StartPath + @"\Weather\Area.txt";
        private readonly string _weatherPath = Model.PM.StartPath + @"\Weather\{0}Weather.txt";//生成天气信息文件，每天只获取一次
        ...
        /// </summary>
        /// <returns></returns>
        public TimedTask.Model.Area GetCurrentArea()
        {
            var file = new FileInfo(_filePath);
            TimedTask.Model.Area result;
            if (!file.Exists)
            {
                //文件不存在就返回一个默认值,默认是成都
                result = new TimedTask.Model.Area();
                result.ID = 250;
                result.Name = "成都";
                result.ZoneID = 23;
            ...
            {
                DataContractJsonSerializer ser = new DataContractJsonSerializer(typeof(TimedTask.Model.Area));
                using (var stream = file.Open(FileMode.Open, FileAccess.Read))
                {
                    result = (TimedTask.Model.Area)ser.ReadObject(stream);
                }
            }
            return result;
        }
        public void SaveCurrentArea(TimedTask.Model.Area area)
        {
            DataContractJsonSerializer ser = new DataContractJsonSerializer(typeof(TimedTask.Model.Area));
            var file = new FileInfo(_filePath);
            FileStream stream;
        ...
        public string[] GetWeather(string cityName)
        {
            ...
            //删除2天前文件
            Helper.Instance.DropFiles(TimedTask.Model.PM.StartPath + "\\Weather\\", cityName, new string[] { ".txt" }, 2);
            ...
            if (File.Exists(path))
            {
                ...
                tmp = Helper.Instance.ReadFile(path);
                weatherInfoList = tmp.Split(TimedTask.Model.PM.SpiderChar);
            }
            ...
            if (weatherInfoList != null && weatherInfoList.Length > 0)
            {
                ...
                {
                    tmp += s + TimedTask.Model.PM.SpiderChar;
                }
                tmp = tmp.Remove(tmp.Length - 1);

                if (!File.Exists(path))
                    Helper.Instance.WriteFile(path, tmp);
            }
            return weatherInfoList;
        }
    }
